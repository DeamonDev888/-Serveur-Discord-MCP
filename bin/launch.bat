...
set "BUILD_CMD=tsc -p tsconfig.build.json"
set "NAME=serveur_discord"
...
powershell -NoProfile -Command "Start-Process -WindowStyle Hidden -FilePath node -WorkingDirectory '%SCRIPT_DIR%' -ArgumentList '--env-file=.env','%ENTRY%' -RedirectStandardOutput '%LOG%\%NAME%.log' -RedirectStandardError '%LOG%\%NAME%.err.log'"
echo   [SPAWN] %NAME%
echo.
