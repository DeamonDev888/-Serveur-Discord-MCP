 ...
 */
export function generateMentionErrorMessage(validation: ReturnType<typeof validateDiscordMentions>, fieldName: string): string {
...
export function validateFieldLength(fields: any[], title?: string, description?: string, footerText?: string): { valid: boolean; warnings: string[]; totalLength: number } {
  ...
  fields?.forEach((field, index) => {
    ...
    if (nameLen > 256) warnings.push(`Champ #${index + 1}: Le nom dépasse 256 caractères (${nameLen})`);
    if (valLen > 1024) warnings.push(`Champ #${index + 1}: La valeur dépasse 1024 caractères (${valLen}) ⚠️`);
  });

  if (totalLength > 6000) warnings.push(`⚠️ TOTAL: L'embed dépasse la limite globale de 6000 caractères (${totalLength}/6000) ! 🛑`);
...
export function generateAsciiChart(type: string, data: number[], labels?: string[], options: any = {}): string {
